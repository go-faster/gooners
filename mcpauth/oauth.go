...
type oauthState struct {
	...
	// fallbackScopes are advertised when the config names none; see scopes().
...
func newOAuthState(cfg OAuthConfig, opts Options) *oauthState {
	if !cfg.Enabled {
		return nil
	}
	...
	return &oauthState{
		...
		name:           opts.Name,
		tokenTTL:       ttl,
		fallbackScopes: opts.Scopes,
		codes:          map[string]oauthCode{},
		tokens:         map[string]auth.TokenInfo{},
	}
}

// serve handles the OAuth endpoints, reporting whether it answered the request.
func (o *oauthState) serve(secret SecretResolver, w http.ResponseWriter, r *http.Request) bool {
	switch r.URL.Path {
...
func (o *oauthState) serveAuthorize(secret SecretResolver, w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		o.renderAuthorize(w, r.URL.RawQuery, "")
	case http.MethodPost:
		o.handleAuthorizePost(secret, w, r)
	default:
		http.Error(w, http.StatusText(http.StatusMethodNotAllowed), http.StatusMethodNotAllowed)
	}
}
...
func (o *oauthState) handleAuthorizePost(secret SecretResolver, w http.ResponseWriter, r *http.Request) {
	...
	if !constantTimeEqual("Bearer "+r.Form.Get("token"), expected) && !constantTimeEqual(r.Form.Get("token"), expected) {
		o.renderAuthorize(w, r.Form.Get("query"), "Invalid token")
...
// requests no explicit scope. cfg.Scopes, if set, overrides the scopes the
// caller derived, for a flat and manually maintained list.
func (o *oauthState) scopes() []string {
	if len(o.cfg.Scopes) > 0 {
		return o.cfg.Scopes
	}
	if len(o.fallbackScopes) > 0 {
		return o.fallbackScopes
	}
...
<html lang="en">
<head><meta charset="utf-8"><title>Authorize {{.Name}}</title></head>
<body>
