...
7. Call `get_dashboard_state` to review layout and coverage. The response includes a `layout` field — a band-format spatial summary showing each row, its panels (left-to-right per Y-band), and any grid gaps. **Check the `layout` field: verify each panel is under the correct row, and that no panels are stranded in the `(no row)` section.** Row IDs appear in each row header line; copy them verbatim when adding or moving panels. Then call `export_dashboard`. Use `save` only when the user explicitly wants to push to Grafana. Use `update_dashboard` to set `refresh`, `tooltip` (shared crosshair), and `description` before export.
