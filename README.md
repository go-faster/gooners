...
| [mcpgateway](cmd/mcpgateway) | MCP proxy that aggregates tools from multiple upstream MCP servers (stdio, streamable-http, sse) with per-upstream prefixes, allow/deny globs, description trim, output redaction, optional per-upstream host/path routes, and lazy tool listing with `search_tools`/`describe_tools` |
