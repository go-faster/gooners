...
func (g *Gateway) Reload(ctx context.Context, cfg *Config) (ReloadResult, error) {
	...
	for _, uc := range slices.Concat(plan.add, plan.restart) {
		u, err := g.newUpstream(uc, resolver, redactor)
		if err != nil {
...
// detachUpstream removes everything an upstream contributed and closes it.
// Syncing it against an empty feature set reuses the same diff/apply path a
// listChanged notification takes, so downstream clients see one ordinary
// removal rather than a special case.
func (g *Gateway) detachUpstream(ctx context.Context, u *Upstream) {
	name := u.cfg.Name
	...
	if err := u.Close(ctx); err != nil {
		g.logger.Warn("close upstream during reload", zap.String("upstream", name), zap.Error(err))
	}
}
